package pay.assignment.sliding_window_counter.aspect
...
import pay.assignment.sliding_window_counter.annotation.RateLimited
import pay.assignment.sliding_window_counter.*

/**
 * Rate Limiting을 위한 AOP Aspect
 * 
 * @RateLimited 어노테이션이 붙은 메서드에 대해 자동으로 Rate Limiting을 적용
 * Sliding Window Counter 알고리즘을 사용하여 사용자별 요청 제한 수행
 */
@Aspect
@Component
class RateLimiterAspect {

    // Sliding Window Counter 기반 Rate Limiter 인스턴스
    // 싱글톤으로 관리되어 모든 요청에 대해 공유 상태 유지
    private val rateLimiter: RateLimiter = SlidingWindowCounterLimiter()
    ...
    fun rateLimit(joinPoint: ProceedingJoinPoint, rateLimited: RateLimited): Any? {
        ...
        // 3. Sliding Window Counter를 통한 허용/거부 결정
